...
public class ModerationTools {
	...
	public static boolean addReport(UUID message, UUID user, long timestamp) {
		return addReport(message, user, timestamp, "");
	}

	public static boolean addReport(UUID message, UUID user, long timestamp, String reason) {
		if (!messageExists(message) || !userExists(user)) return false;
		return ReportRegistry.getInstance().addReport(message, user, timestamp, reason);
	}
